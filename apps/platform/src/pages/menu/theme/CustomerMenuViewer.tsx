...
export default function CustomerMenuViewer({
  ...
  React.useEffect(() => {
    if (menuData) {
      ...
      console.log(
        "🔍 CustomerMenuViewer - Menu items with image URLs:",
      ...
      const itemsWithImages = menuData.items?.filter(
        (item: any) => item.image_url
      );
      console.log(
        "🔍 CustomerMenuViewer - Items that have images:",
        itemsWithImages?.length || 0
      );
      if (itemsWithImages && itemsWithImages.length > 0) {
        console.log(
          "🔍 CustomerMenuViewer - First item with image:",
          itemsWithImages[0]
        );
      }
    }
      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-50 to-gray-100 flex items-center justify-center px-6">
        <div className="text-center max-w-md">
          ...
          <button
            ...
            className="px-6 py-3 bg-slate-900 text-white rounded-xl font-medium hover:bg-slate-800 transition-all shadow-lg hover:shadow-xl transform hover:scale-105 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-slate-900/50"
          >
            Try again
          </button>
        </div>
...
// Add styles for scrollbar hiding (and any future utilities)
const styles = `
.scrollbar-hide {
